...
def get_user_team_role(member: discord.Member) -> discord.Role | None:
    # 1) Try teams.json mapping (preferred)
    try:
        teams = load_teams()
        role_ids = {int(e.get("role_id")) for e in teams if e.get("role_id")}
    ...
    # 2) Try existing is_team_role logic
    guild = member.guild
    for r in member.roles:
        try:
    ...
    # 3) Fallback: return first non-default, non-managed, non-protected role the member has
    protected = {
        CAPTAIN_ROLE_ID, CO_CAPTAIN_ROLE_ID, TEAM_PLAYER_ROLE_ID, TEAM_EXEC_ROLE_ID,
        HEAD_REF_ROLE_ID, REF_ROLE_ID, HEAD_CASTER_ROLE_ID, CASTER_ROLE_ID,
        STREAM_WATCHER_ROLE_ID, UNBORN_CAPTAIN_ROLE_ID, EVENT_PING_ROLE_ID,
    ...
    for r in member.roles:
        if r.is_default() or r.managed or r.id in protected:
            continue
        return r
...
class AddScrimModal(discord.ui.Modal, title="Add Scrim"):
    ...
    def _resolve_team(self, guild: discord.Guild, raw: str) -> tuple[str,str]:
        text = raw.strip()
        if text.startswith("<@&") and text.endswith(">"):
            try:
                rid = int(text.strip("<@&>"))
                r = guild.get_role(rid)
                if r: return r.mention, r.name
        ...
        r = discord.utils.get(guild.roles, name=text) or discord.utils.find(lambda rr: rr.name.lower()==text.lower(), guild.roles)
        if r: return r.mention,r.name
        return text,text
    async def on_submit(self, interaction: discord.Interaction):
        guild = interaction.guild
        ...
        if not isinstance(cat, discord.CategoryChannel):
            await interaction.response.send_message("Scrim category is not configured.", ephemeral=True); return
        t1m,t1n = self._resolve_team(guild,self.team1.value)
        t2m,t2n = self._resolve_team(guild,self.team2.value)
        is_lb = self.lower_bracket.value.strip().lower() in ("true","yes","y","1")

        raw_name = f"{t1n} vs {t2n}"
        # sanitize channel name: remove invalid chars, replace spaces with hyphens, lowercase
        chan_name = re.sub(r"[^a-zA-Z0-9 _-]", "", raw_name).strip().replace(" ", "-").lower()[:90]
        channel_topic = f"{t1n} Vs {t2n}"

        overwrites={guild.default_role:discord.PermissionOverwrite(view_channel=False)}
        ch = await guild.create_text_channel(name=chan_name,category=cat,overwrites=overwrites,
                                             topic=channel_topic,
...
class LeaveCog(commands.Cog):
    ...
    @app_commands.command(name="leave", description="Leave your team (players, co-captains).")
...
class AdminPanelView(discord.ui.View):
    ...
    async def disband_all(self, interaction, button):
        ...
        for r in list(guild.roles):
            if r.is_default() or r.managed or r.id in protected:
                continue
            try:
                await r.delete(reason=f"Disband-all by {interaction.user}")
            except Exception:
                pass
        ids_strip = [CAPTAIN_ROLE_ID, CO_CAPTAIN_ROLE_ID, TEAM_PLAYER_ROLE_ID, TEAM_EXEC_ROLE_ID]
        for m in guild.members:
            rs = [r for r in m.roles if r.id in ids_strip]
            if rs:
                try:
                    await m.remove_roles(*rs, reason="Disband-all admin action")
                except Exception:
                    pass
        if isinstance(tx, discord.TextChannel):
            try:
                await tx.send("# ALL TEAMS HAS BEEN DISBANDED")
            except Exception:
                pass
        await interaction.followup.send("All teams disbanded and roles removed.", ephemeral=True)
    ...
    async def submit_score(self, interaction, button):
        # Use the global SEEDING_OPEN flag to choose modal
        global SEEDING_OPEN
        if SEEDING_OPEN:
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        class InviteAcceptView(discord.ui.View):
            ...
            async def accept(self, intr: discord.Interaction, btn: discord.ui.Button):
                ...
                try:
                    await intr.response.send_message(
                        f"You joined {team_name}!",
                        ephemeral=True,
                    )
                except Exception:
                    pass

                # react on the DM message
                try:
        ...
        embed = discord.Embed(
            ...
            color=team_role.colour or discord.Color.blurple(),
        )
        try:
...
class AssignmentClaimView(discord.ui.View):
    ...
    async def claim_caster(self, interaction: discord.Interaction, button: discord.ui.Button):
        # allow head caster or caster to claim
        ...
        allowed = any(has_role_id(user, rid) for rid in (HEAD_CASTER_ROLE_ID, CASTER_ROLE_ID))
        if not allowed:
            await interaction.response.send_message("You must have a caster role to claim Caster.", ephemeral=True)
            return

        # If someone else had the caster slot, remove them (override)
        prev = self.caster
        self.caster = user

        # disable only this button for this view message
        button.disabled = True
        # Ensure the corresponding Ref button remains in its state

        await interaction.response.send_message("You claimed Caster.", ephemeral=True)
        # If previous exists and is not the same user, notify previous (best-effort)
    ...
    async def claim_ref(self, interaction: discord.Interaction, button: discord.ui.Button):
        # allow head ref or ref to claim
        user = interaction.user
        guild = interaction.guild
        if guild is None:
            await interaction.response.send_message("Use this in a server.", ephemeral=True)
        ...
        allowed = any(has_role_id(user, rid) for rid in (HEAD_REF_ROLE_ID, REF_ROLE_ID))
        if not allowed:
            await interaction.response.send_message("You must have a referee role to claim Referee.", ephemeral=True)
            return
...
class StatsCog(commands.Cog):
    ...
    async def _create_stats_once(self, guild: discord.Guild):
        print(f"[STATS] _create_stats_once for {guild.name} ({guild.id})")
        if guild.id in self._initialized_guilds:
            print(f"[STATS] Already initialized for {guild.name}")
            return
        ...
        # find existing category safely
        try:
            category = discord.utils.get(guild.categories, name=category_name)
        except Exception as e:
            print(f"[STATS] Failed to list categories in {guild.name}: {e}")
            category = None
        ...
        if category is None:
            try:
                ...
                print(f"[STATS] Created category {category_name} in {guild.name}")
            except discord.Forbidden:
                print(f"[STATS] Missing Manage Channels permission in {guild.name} (cannot create category)")
                return
            except Exception as e:
                print(f"[STATS] Failed to create category in {guild.name}: {e}")
                return
        else:
            print(f"[STATS] Found existing category {category_name} in {guild.name}")

        # move category to top (best effort)
        try:
            await category.edit(position=0)
        except Exception as e:
            print(f"[STATS] Failed to move category to top in {guild.name}: {e}")

        # locked-but-visible overwrites for @everyone
        overwrites = {
        ...
        if member_ch is None:
            try:
                member_ch = await guild.create_text_channel(name=f"{member_prefix}: 0", category=category, overwrites=overwrites, reason="Create member stats channel")
                print(f"[STATS] Created member stats channel in {guild.name}")
            except discord.Forbidden:
                print(f"[STATS] Missing Manage Channels permission in {guild.name} (cannot create member channel)")
            except Exception as e:
                print(f"[STATS] Failed to create member channel in {guild.name}: {e}")
        else:
            print(f"[STATS] Found existing member stats channel {member_ch.name} in {guild.name}")
            try:
                await member_ch.edit(category=category, overwrites=overwrites)
            except Exception as e:
        ...
        if team_ch is None:
            try:
                team_ch = await guild.create_text_channel(name=f"{team_prefix}: 0", category=category, overwrites=overwrites, reason="Create team stats channel")
                print(f"[STATS] Created team stats channel in {guild.name}")
            except discord.Forbidden:
                print(f"[STATS] Missing Manage Channels permission in {guild.name} (cannot create team channel)")
            except Exception as e:
                print(f"[STATS] Failed to create team channel in {guild.name}: {e}")
        else:
            print(f"[STATS] Found existing team stats channel {team_ch.name} in {guild.name}")
            try:
                await team_ch.edit(category=category, overwrites=overwrites)
            except Exception as e:
                print(f"[STATS] Failed to edit team channel in {guild.name}: {e}")

        # mark initialized if at least one channel is present
        if category and (member_ch or team_ch):
            self._initialized_guilds.add(guild.id)
            print(f"[STATS] Initialization complete for {guild.name}")
        else:
            print(f"[STATS] Initialization incomplete for {guild.name} (member_ch={member_ch}, team_ch={team_ch})")
    ...
    async def _update_guild_stats(self, guild: discord.Guild):
        ...
        category = discord.utils.get(guild.categories, name=category_name)
        if category is None:
            print(f"[STATS] No stats category found in {guild.name}")
            return
        ...
        if member_ch is None or team_ch is None:
            print(f"[STATS] Missing stats channels in {guild.name} (member_ch={member_ch}, team_ch={team_ch})")
            return

        # ensure member cache: use guild.member_count fallback to len(cache)
        ...
        print(f"[STATS] {guild.name}: member_count={member_count} (guild.member_count={guild.member_count}, cache={cache_count})")
        ...
        print(f"[STATS] {guild.name}: team_count={valid_team_count}")
        ...
        if guild.me.guild_permissions.manage_channels:
            try:
                if member_ch.name != desired_member_name:
                    ...
                    print(f"[STATS] Updated member channel name in {guild.name} to {desired_member_name}")
            except Exception as e:
                print(f"[STATS] Failed to edit member channel in {guild.name}: {e}")
            ...
            try:
                if team_ch.name != desired_team_name:
                    ...
                    print(f"[STATS] Updated team channel name in {guild.name} to {desired_team_name}")
            except Exception as e:
                print(f"[STATS] Failed to edit team channel in {guild.name}: {e}")
        else:
            print(f"[STATS] Missing Manage Channels when updating in {guild.name}")
    ...
    async def update_stats_task(self):
        ...
        for guild in self.bot.guilds:
            try:
                ...
                print(f"[STATS] Error updating stats in {guild.name}: {e}")
                continue
    ...
    async def before_update_stats(self):
        ...
        for guild in self.bot.guilds:
            try:
                await guild.chunk()
                print(f"[STATS] Chunked members for {guild.name}")
            except Exception as e:
                print(f"[STATS] Failed to chunk members for {guild.name}: {e}")
            try:
                await self._create_stats_once(guild)
            except Exception as e:
                print(f"[STATS] Error during initial stats setup in {guild.name}: {e}")
                continue
    ...
    async def on_guild_join(self, guild: discord.Guild):
        try:
            await guild.chunk()
            print(f"[STATS] Chunked members on guild join for {guild.name}")
        except Exception as e:
            print(f"[STATS] Failed to chunk members on join for {guild.name}: {e}")
        try:
            await self._create_stats_once(guild)
        except Exception as e:
            print(f"[STATS] Error creating stats on guild join in {guild.name}: {e}")
...
class RosterCog(commands.Cog):
    ...
    async def roster(self, interaction: discord.Interaction):
        ...
        # Prefer the caller's team if they have one
        member = interaction.user
        team_role = get_user_team_role(member)

        # Fallback: first detected team role in the guild
        if team_role is None:
            protected = {
                CAPTAIN_ROLE_ID, CO_CAPTAIN_ROLE_ID, TEAM_PLAYER_ROLE_ID, TEAM_EXEC_ROLE_ID,
                HEAD_REF_ROLE_ID, REF_ROLE_ID, HEAD_CASTER_ROLE_ID, CASTER_ROLE_ID,
            ...
            candidate_roles = [
                ...
                if not r.is_default() and not r.managed and r.id not in protected
            ]
            teams = [r for r in candidate_roles if is_team_role(guild, r)]
            team_role = teams[0] if teams else None
        ...
        embed_color = team_role.colour if getattr(team_role, "colour", None) else discord.Color.dark_green()
        embed = discord.Embed(
        ...
        # uses the same formatter as /manage-team and /admin-manage
        embed.add_field(name="Team Executive", value=format_list_arrow([data["executive"]]), inline=False)
        embed.add_field(name="Captain",        value=format_list_arrow([data["captain"]]),   inline=False)
        embed.add_field(name="Co-Captains",    value=format_list_arrow(data.get("co_captains", [])), inline=False)
...
class CommandGuideCog(commands.Cog):
    ...
    async def on_ready(self):
        ...
        for guild in self.bot.guilds:
            ...
            embed.set_footer(text="GTCC Season Management System")
